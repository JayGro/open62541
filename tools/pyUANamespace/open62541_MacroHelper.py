...
class open62541_MacroHelper():
  ...
  def getCreateNode(self, node, parentNode, parentReference, unprintedNodes):
    code = []
    ...
    code.append("UA_NodeId parentReferenceNodeId = " +
                str(self.getCreateNodeIDMacro(parentReference.referenceType() if parentReference != None else None)) + ";")
    extrNs = node.browseName().split(":")
    if len(extrNs) > 1:
